...
def main(input):
    ...
    for i in input:
        ...
        rng[1] = int(rng[1])
        letter = tmp[1][0]
        password = [char for char in tmp[2]]
        newStr = [n for n in password[rng[0]-1]+password[rng[1]-1]]
        print(password, newStr)
        if newStr.count(letter) == 1:
            counter += 1
    return counter
...
def main2(input):
    counter = 0
    for i in input:
        [count, letter, passwrd] = i.split(" ")
        count = count.split("-")
        if int(count[0]) <= passwrd.count(letter[0]) <= int(count[1]):
            counter += 1
    return counter
