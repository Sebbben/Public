...
def validate(passport):
    # check if contains
    if not all(check in passport for check in ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]):
        return False
    ...
    for i in passport:
        pair = i.split(":")
        passportDict[pair[0]] = pair[1]

    # check birth year, issue year and expiration year
    if not (
        (1920 <= int(passportDict["byr"]) <= 2002) and
        (2010 <= int(passportDict["iyr"]) <= 2020) and
        (2020 <= int(passportDict["eyr"]) <= 2030)
    ):
        return False

    if not(((passportDict["hgt"].endswith("cm")) and (150 <= int(passportDict["hgt"][0:-2]) <= 193)) or ((passportDict["hgt"].endswith("in")) and (59 <= int(passportDict["hgt"][0:-2]) <= 76))):
        return False

    import re
    if not (passportDict["hcl"][0] == "#" and re.sub('[^a-fA-F0-9]', '', passportDict["hcl"][1:]) == passportDict["hcl"][1:]):
        return False

    if not (any(color in passportDict["ecl"] for color in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"])):
        return False
