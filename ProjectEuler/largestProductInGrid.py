...
def main(biggestValue):
    for i in range(len(input)):
        for j in range(len(input[i])):
            if(j <= len(input[i])-4 and i <= len(input)-4):
                biggestValue = max(biggestValue, input[i][j]*input[i+1][j+1]*input[i+2][j+2]*input[i+3][j+3])
            if(j <= len(input[i])-4 and i >= 3):
                biggestValue = max(biggestValue, input[i][j]*input[i-1][j+1]*input[i-2][j+2]*input[i-3][j+3])
            if(j <= len(input[i])-4):
                biggestValue = max(biggestValue, input[i][j]*input[i][j+1]*input[i][j+2]*input[i][j+3])
            if(i <= len(input)-4):
                biggestValue = max(biggestValue, input[i][j]*input[i+1][j]*input[i+2][j]*input[i+3][j])
    return biggestValue            


biggestValue = 0
print(main(biggestValue))
#51 267 216